...
static ssize_t device_read(struct file *, char __user *buffer, size_t len, loff_t *offset); 
...
static int device_release(struct inode *, struct file *) {
	pr_info("pacekt_sniffer: Device closed");
	return 0;	
}

static ssize_t device_read(struct file *, char __user *user_buffer, size_t len, loff_t *offset) {
	int size_to_copy;
...
static unsigned int capture(void *priv, struct sk_buff *skb, const struct nf_hook_state *state) {
	...
	if(buffer_index >= BUFFER_SIZE - 1024) {
		buffer_index = 0;
	...
	buffer_index += scnprintf(buffer + buffer_index, 128, "Packet: %s -> %s\n", src_ip, dest_ip);

	switch(ip_header->protocol) {
		case IPPROTO_TCP:
			tcp_header = tcp_hdr(skb);
			buffer_index += scnprintf(buffer + buffer_index, 128, "TCP: %s:%d -> %s:%d\n", src_ip, ntohs(tcp_header->source), dest_ip, ntohs(tcp_header->dest));
		break;
		case IPPROTO_UDP:
			udp_header = udp_hdr(skb);
			buffer_index += scnprintf(buffer + buffer_index, 128, "UDP: %s:%d -> %s:%d\n", src_ip, ntohs(udp_header->source), dest_ip, ntohs(udp_header->dest));
		break;
	}
...
static int __init  packet_sniffer_init(void) {
	...
	if(ret) {
		pr_err("packet_sniffer: failed to add char. device");
	...
	if(IS_ERR(packet_class)) {
		pr_err("packet_sniffer: failed to create a device class");
